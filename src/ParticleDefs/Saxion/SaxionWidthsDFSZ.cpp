...
std::vector<Models::PartialWidth> SaxionWidthsDFSZ::widthVBosonVBoson(){
    ...
    std::vector<Models::PartialWidth> widths;
    widths.push_back(widthZBosonZBoson(zBoson, ggZ, gsVV));
    widths.push_back(widthWBosonWBoson(wBoson, ggW, gsVV));
    return widths;
}

std::vector<Models::PartialWidth> SaxionWidthsDFSZ::widthVBosonHiggs(){
...
// from arXiv:1309.5365
std::vector<Models::PartialWidth> SaxionWidthsDFSZ::widthFermionFermion(){
    double alpha = model_.Susy.MixingAngles.alfah;
    double beta = atan(model_.Susy.TanBeta);
    // couplings from A.12
...
std::vector<Models::PartialWidth> SaxionWidthsDFSZ::widthNeutralinoNeutralino(double cH){
...
std::vector<Models::PartialWidth> SaxionWidthsDFSZ::widthSneutrinoSneutrino(){
...
std::vector<Models::PartialWidth> SaxionWidthsDFSZ::widthSleptonSlepton(){
...
std::vector<Models::PartialWidth> SaxionWidthsDFSZ::widthSquarkSquark(){
...
std::vector<Models::PartialWidth> SaxionWidthsDFSZ::widthDiHiggs(double cH){
    auto higgsLight = ModelBaseOps::FindConst(particles_, "higgslight");
    auto higgsHeavy = ModelBaseOps::FindConst(particles_, "higgsheavy");
    auto higgsPseudo = ModelBaseOps::FindConst(particles_, "higgspseudoscalar");
    auto higgsCharged = ModelBaseOps::FindConst(particles_, "higgscharged");
...
double SaxionWidthsDFSZ::widthCharginoCharginoDifferentGen(double mSaxion, double mChargino1, double mChargino2, double gSU2, double sigCoupling, double piCoupling){
    ...
    if (abs(mSaxion) > (abs(mChargino1) + abs(mChargino2))){
        wCC = 2. * pow(gSU2, 2.) / (16. * M_PI) * abs(mSaxion) * sqrt( psLamb(1., pow(mChargino1, 2.) / pow(mSaxion, 2.), pow(mChargino2, 2.) / pow(mSaxion, 2.)) ) * ( pow(sigCoupling, 2.) * ( 1. - pow((abs(mChargino1) + abs(mChargino2)) / mSaxion, 2.) ) + pow(piCoupling, 2.) * ( 1. - pow( (abs(mChargino2) - abs(mChargino1)) / mSaxion, 2. ) ) );
    }
    return wCC;
}

// helper method to calculate all couplings and return total width for same gen charginos total
std::vector<Models::PartialWidth> SaxionWidthsDFSZ::widthCharginoCharginoSameGen(double cH){
